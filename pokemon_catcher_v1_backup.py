...
def screenshot_adb_cached(force_fresh: bool = False) -> Optional[np.ndarray]:
    ...
    if not force_fresh and _screenshot_cache is not None:
...
def obtener_pokebola_optima() -> str:
    ...
    with _lock_pokebola_stats:
        # Retorna el tipo con mayor tasa de éxito, con "boldness" sobre tipos menos probados
        return max(_pokebola_tasa_exito.items(), key=lambda x: x[1] + random.uniform(0, 0.05))[0]
    ...
    while _diagnostico_activo:
        try:
            ...
            if _tmpl_camara is not None:
                try:
                    roi_cam = img[int(h * 0.05):int(h * 0.30), int(w * 0.30):int(w * 0.70)]
            ...
            if not template_detectado:
                if pct_rojo > 0.025 and pct_blanco < 0.30:
                    nuevo_estado = "COMBATE"
            ...
            # Debug: mostrar análisis detallado cuando cambia
            if DEBUG and nuevo_estado != _estado_diagnostico:
                console.print(f"[dim cyan][DIAG] Cambio: {_estado_diagnostico} → {nuevo_estado} (blanco={pct_blanco:.1%}, verde={pct_verde:.1%}, rojo={pct_rojo:.1%})[/]")
            
            # Actualizar estado global con lock
            # SI EL DIAGNÓSTICO ESTÁ CONGELADO, NO actualizar (evita confusión durante impacto)
            with _lock_diagnostico:
                if not _diagnostico_congelado:
                    _estado_diagnostico = nuevo_estado
...
def _seleccionar_pokebola_siguiente() -> str:
    """
    Selecciona la siguiente pokébola a usar basándose en la actual.
    Si falló demasiadas veces, pasa a la siguiente en la secuencia.
    ...
    with _lock_pokebola:
        # Si falló varias veces, cambiar a siguiente
        if _pokebola_intentos_fallidos >= _POKEBOLA_MAX_INTENTOS_ANTES_CAMBIO:
            ...
            idx_siguiente = (idx_actual + 1) % len(secuencia)
            _pokebola_favorita = secuencia[idx_siguiente]
            _pokebola_intentos_fallidos = 0
            console.print(f"[yellow]🎯 Cambiando pokébola a: {_pokebola_favorita}[/]")
